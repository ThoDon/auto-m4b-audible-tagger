...
class AudiobookDatabase:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Create indexes
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_audiobooks_status ON audiobooks(status)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_audiobooks_file_path ON audiobooks(file_path)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_search_sessions_file_id ON search_sessions(file_id)")
    ...
    def add_audiobook(self, file_path: Path, file_id: str) -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute("SELECT file_id FROM audiobooks WHERE file_path = ?", (str(file_path),))
                ...
                """, (
                    ...
                    self._generate_file_hash(file_path)
                ))
                
                conn.commit()
                self.logger.info(f"Added audiobook to database: {file_path} -> {file_id}")
                return True
    ...
    def update_audiobook_status(self, file_id: str, status: str, metadata: Optional[Dict] = None, 
                               cover_path: Optional[str] = None, final_path: Optional[str] = None,
    ...
    def save_search_session(self, session_id: str, file_id: str, search_results: List[Dict]) -> bool:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.commit()
                self.logger.info(f"Saved search session {session_id} for file {file_id}")
                return True
