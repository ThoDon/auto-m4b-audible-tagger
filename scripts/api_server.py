...
class AudiobookAPI:
    ...
    def __init__(self, input_folder: str, output_folder: str, config_path: str = "config.json"):
    ...
    def run_cleanup(self):
        ...
        try:
            ...
            temp_files, invalid_files, empty_folders = cleanup_incoming_folder(str(self.input_folder), auto_mode=True)
            self.logger.info(f"🧹 Automatic cleanup completed: {temp_files} temp files, {invalid_files} invalid files, {empty_folders} empty folders removed")
        except Exception as e:
    ...
    def register_routes(self):
                ...
                for audiobook in audiobooks:
                    ...
                    if file_path.exists():  # Only include files that still exist
                        ...
                        books_info.append({
                            ...
                            'filename': str(relative_path),  # Include subfolder path
                            'path': str(file_path),
                            'size': audiobook['file_size'],
                            'status': audiobook['status'],
                            'parsed_title': title,
                            'parsed_author': author,
                            'search_query': title if author == "Unknown Author" else f"{title} {author}",
        ...
        def search_audiobook(file_id: str):
            ...
            try:
                ...
                for i, result in enumerate(results):
                    ...
                    result_with_id['display_text'] = f"{result.get('title', 'Unknown')} by {result.get('author', 'Unknown')} ({result.get('narrator', 'Unknown narrator')})"
        ...
        def custom_search_audiobook(file_id: str):
            ...
            try:
                ...
                error_details = traceback.format_exc()
                self.logger.error(f"Error in custom search for audiobook {file_id}: {e}")
                self.logger.error(f"Full traceback: {error_details}")
                return jsonify({
                    'status': 'error',
                    'message': str(e),
        ...
        def process_audiobook(file_id: str):
            ...
            try:
                ...
                details = self.tagger.get_book_details(
                    selected_result['asin'], 
                    selected_result['locale']
                )
                
                if not details:
                    self.db.update_audiobook_status(file_id, "error", error_message=f"Could not get details for ASIN: {selected_result['asin']}")
                    return jsonify({
                        'status': 'error',
                ...
                if details.get('cover_url'):
                    cover_path = self.tagger.download_cover(details['cover_url'], details['asin'])
                
                # Tag the file
                try:
                    if not self.tagger.tag_file(file_path, details, cover_path):
                        self.db.update_audiobook_status(file_id, "error", error_message="Failed to tag file")
                        return jsonify({
                            'status': 'error',
                    ...
                    self.db.update_audiobook_status(file_id, "error", error_message=f"Tag error: {str(tag_error)}")
                    return jsonify({
                        'status': 'error',
                ...
                # Move to library
                try:
                    result_path = self.tagger.move_to_library(file_path, details, cover_path)
                    ...
                    self.db.update_audiobook_status(file_id, "error", error_message=f"Move error: {str(e)}")
                    return jsonify({
                        'status': 'error',
        ...
        def skip_audiobook(file_id: str):
            ...
            try:
                ...
                self.db.update_audiobook_status(file_id, "skipped", final_path=str(skipped_path))
